...
namespace Raven.DependencyInjection
{
    internal class LoadInjector
    {
        internal static void Load(IServiceCollection serviceCollection, ISearcher<Assembly> searcher, string[] patterns)
        {
            ...
            var RavenAssemblies = assemblies.Where(d => d.FullName.StartsWith("Raven."));
            var otherAssemblies = assemblies.Where(d => !d.FullName.StartsWith("Raven."));
            foreach (var assembly in RavenAssemblies.Concat(otherAssemblies))
            {
