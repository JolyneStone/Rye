...
namespace KiraNet.AlasFx.Jwt
{
        ...
        where TEntity: UserPermission<TUserKey,TRoleKey>, new()
    {
        ...
        public virtual Task<JsonWebToken> RefreshToken(string refreshToken)
        {
            ...
            {
                ValidIssuer = _jwtOptions.Issuer ?? "AlasFx Identity",
                ValidAudience = _jwtOptions.Audience ?? "AlasFx Client",
            ...
            if (clientId == null)
            {
                throw new AlasFxException("RefreshToken中不包含ClientId声明");
            }
        ...
        protected (string, DateTime) CreateTokenCore(IEnumerable<Claim> claims, JwtOptions options, JwtTokenType tokenType)
        {
            ...
            if (secret == null)
            {
                throw new AlasFxException("创建JwtToken时Secret为空，请在AlasFx:Jwt:Secret节点中进行配置");
            }
