...
namespace Raven.Jwt
{
        ...
        where TEntity: UserPermission<TUserKey,TRoleKey>, new()
    {
        ...
        public virtual Task<JsonWebToken> RefreshToken(string refreshToken)
        {
            ...
            {
                ValidIssuer = _jwtOptions.Issuer ?? "Raven Identity",
                ValidAudience = _jwtOptions.Audience ?? "Raven Client",
            ...
            if (clientId == null)
            {
                throw new RavenException("RefreshToken中不包含ClientId声明");
            }
        ...
        protected (string, DateTime) CreateTokenCore(IEnumerable<Claim> claims, JwtOptions options, JwtTokenType tokenType)
        {
            ...
            if (secret == null)
            {
                throw new RavenException("创建JwtToken时Secret为空，请在Raven:Jwt:Secret节点中进行配置");
            }
