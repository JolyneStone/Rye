    ...
    public class DbProvider : IDbProvider
    {
        ...
        public IUnitOfWork GetUnitOfWork(Type dbContextType, string dbName = null)
        {
            ...
            else
            {
                ...
                var dbConnectionOptionsMap = _serviceProvider.GetRequiredService<IOptions<Raven.Options.RavenOptions>>().Value.DbConnections;
                if (dbConnectionOptionsMap == null || dbConnectionOptionsMap.Count <= 0)
                {
                    throw new RavenException("无法获取数据库配置");
                }
                ...
                {
                    throw new RavenException("无法获取匹配的DbContextOptionsBuilder");
                }
                ...
                if (dbUser == null)
                {
                    throw new RavenException($"无法解析类型为“{dbConnectionOptions.DatabaseType}”的 {typeof(IDbContextOptionsBuilderUser).FullName} 实例");
                }
                ...
                else
                {
                    ...
                    {
                        throw new RavenException("无法获取有效的上下文构造器");
                    }
